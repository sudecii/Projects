...
## 🏨 Booking Review Score Prediction — NLP + Regression  
**TL;DR:** Модель прогнозирует итоговый рейтинг отеля на основе текстовых отзывов и характеристик проживания. Использует NLP-предобработку и векторизацию TF-IDF.

### 🎯 Бизнес-задача  
Платформы бронирования стремятся улучшить качество сервиса и анализ отзывов:  
- выявлять проблемные отели,  
- определять факторы, влияющие на рейтинг,  
- автоматизировать обработку больших массивов отзывов.  

**Ценность:** ускорение анализа обратной связи и повышение прозрачности качества для пользователей.
...
### 🧠 Стек & навыки  
NLP-предобработка, лемматизация, TF-IDF, визуализация данных, регрессионные модели (RandomForest, GradientBoosting, Linear Models).

📁 Папка: [ml-booking](./ml-booking)
